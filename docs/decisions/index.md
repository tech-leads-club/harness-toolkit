...
| [AD-145](/decisions/ad-145.md) | Codex capability flags are read off the vendor's hook reference, and the four it cannot settle fail safe | active |
| [AD-146](/decisions/ad-146.md) | opencode's two plugin API generations get two capability tables, and the rows they disagree on are not the ones the design predicted | active |
| [AD-147](/decisions/ad-147.md) | VS Code capability flags come from the VS Code hooks page for what a hook returns, and from the Copilot reference only for what a hook receives | active |
| [AD-148](/decisions/ad-148.md) | The VS Code writer ships complete and is never dispatched, because Agent Hooks are Preview and the hook file schema is unpublished | active |
| [AD-149](/decisions/ad-149.md) | A declared reason may name which reason it is, so the comment gate matches the code it polices | active |
| [AD-150](/decisions/ad-150.md) | Every provider wiring writer backs up the target file before overwriting it | active |
