...
> **Correction, 2026-08-19.** Item 1 is no longer true, and nothing here caused it. Making the repository public
> switched on a branch ruleset requiring one approving review, which a bot cannot give itself
> ([/decisions/ad-083.md](/decisions/ad-083.md)), and the `publish` environment was later given required
> reviewers. A release now needs two human approvals: the release PR and the deployment. The mechanism below is
> intact — the bot still assembles and merges — but the claim that a `fix:` reaches npm with no human step is
> false, and the trade-offs written on that premise should be read as describing a configuration this repository
> no longer has.
