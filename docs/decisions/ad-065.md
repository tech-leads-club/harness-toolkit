...
**Thirteen kinds are orphans right now.** Emitted and read by no declared consumer — `gate.outcome`,
`policy.deny`, `session.start` and ten more. Most are read by the session report and by `why`, which are not
registered as consumers, so the list overstates. That is the honest first reading, and shrinking it means
registering those consumers.

## Not decided here

- **Registering the report and `why` as consumers.** It would shrink the orphan list to the genuinely unread
  and make the report meaningful. Worth doing next.
