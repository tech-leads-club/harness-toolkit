...
- **Uninstalling on Windows.** `install.ps1` writes the same artefacts and this command reads POSIX symlinks. The
  un-merge and the payload removal are platform-neutral; the link handling is not.
