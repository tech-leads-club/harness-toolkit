...
`tools/check-screens.ts` is a gate step. It finds every exported `*Text` renderer and reports the ones that do
not go through the shared renderer, against a budget that may only fall. Converting fourteen screens in one
change would be unreviewable, and a checker that merely allowed a list would hide the remainder — so the
remainder is printed on every run.

## Not decided here

- **The ten screens still on their own strings.** Named by `check-screens` on every gate run: `helpText`,
  `pendingText`, `policyText`, `pricesHelpText`, `versionText`, `usageText`, `installReportText`,
  `gardenText`, `listText`, `liveText`.
- **`obs report`.** Its output is written to a `.md` file *and* printed, so styling the string would put escapes
  in the artifact. It needs a terminal rendering separate from the Markdown one, which is a change to what it
  produces rather than to how it looks.
