...
const PR_OPEN_HINT = TRIGGER_ESCAPE_HATCH_HINT["pr-open"] as string;
...
describe("ruleMessage", () => {
  ...
  test("a rule with no body still says which rule and what is missing", () => {
    assert.equal(
      ruleMessage(rule({ body: "", on: { kind: "commit" } }), ["gate(test) since HEAD"], "/repo", "abc1234"),
  ...
  test("DIAG-11 the message points at tlc harness why, whether or not the rule has a body", () => {
    ...
    // why: the pointer sits before the pr-open hint and the operator's own body, right after AD-120's own
    // checked-root/sha line — not appended at the very end, where it would read as part of the operator's prose.
    assert.equal(
      withBody,
      `rule review-before-pr (global): missing subagent(the-jury) since HEAD — checked /repo at abc1234\n${WHY_POINTER}\n\n${PR_OPEN_HINT}\n\nConvene the jury.\nChecklist: docs/review-checklist.md`,
    );
  });

  /**
   * AC1/AC2 — the draft escape hatch is generated by the harness itself, not by anything the operator wrote
   * ([/decisions/ad-138.md](/decisions/ad-138.md)).
   */
  test("AC1/AC2 a pr-open rule's message carries the draft escape hatch even with no body of its own", () => {
    const message = ruleMessage(rule({ body: "" }), ["subagent(the-jury) since HEAD"], "/repo", "abc1234");
  ...
  /** AC4 — the hint sits between the harness's own head and the operator's body; the body itself is untouched. */
  test("AC4 the operator's body still renders verbatim, immediately after the hint", () => {
    const message = ruleMessage(rule(), ["subagent(the-jury) since HEAD"], "/repo", "abc1234");

    assert.equal(
      message.endsWith(`${PR_OPEN_HINT}\n\nConvene the jury.\nChecklist: docs/review-checklist.md`),
      true,
    );
  });
});
