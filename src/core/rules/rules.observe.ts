...
export function observationFrom(event: ObservableEvent, context: ObserveContext): Observation | null {
  const base = { sha: context.sha, sessionKey: context.sessionKey, at: context.at };

  if (event.event === "subagent.stop") {
    // why the type and not the id: a rule says "the jury reviewed", not "agent 7f3a reviewed".
    return event.subagentType === undefined ? null : { kind: "subagent", value: event.subagentType, ...base };
  }

  if (event.event === "tool.after" || event.event === "shell.after") {
    return event.command === undefined ? null : { kind: "command", value: event.command, ...base };
  }

  if (event.event === "edit.after") {
    return event.filePath === undefined ? null : { kind: "file", value: event.filePath, ...base };
  }
