...
var stopHandler = async (event, ctx) => {
  ...
  const skipVerify = existsSync25(join29(flagsDir(root), "skip-verify"));
...
`)
  ...
  if (codeTargets.length > 0 && coreFacade.observe.shouldObserve(policy.observe, "comments", policy.comments.enabled)) {
    const hits = await coreFacade.commentPolicy.scanAddedComments(root, codeTargets, policy.comments.mode, turnBase);
    ...
    if (manifests.length > 0) {
      ...
      const outcome = coreFacade.supplyChain.inspectSupplyChain({
        ...
        readManifest: (relativePath) => {
          try {
            return readFileSync27(join29(root, relativePath), "utf8");
          } catch {
            return null;
          }
        }
