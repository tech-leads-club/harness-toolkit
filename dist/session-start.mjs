...
function claudeMdPath(root) {
  return join26(root, "CLAUDE.md");
}
function renderClaudeLessonsView(root) {
  if (!existsSync25(claudeLessonsSourcePath(root))) {
    return null;
  }
  const path = claudeMdPath(root);
  const existing = existsSync25(path) ? readFileSync27(path, "utf8") : "";
...
import { existsSync as existsSync26, mkdirSync as mkdirSync17, readFileSync as readFileSync28, writeFileSync as writeFileSync16 } from "node:fs";
import { dirname as dirname8, join as join27 } from "node:path";
function cursorLessonsSourcePath(root) {
  return join27(dirname8(projectConfigPath(root)), "lessons.md");
}
function cursorLessonsViewPath(root) {
  return join27(root, ".cursor", "rules", "harness-lessons.mdc");
}
function renderCursorLessonsView(root) {
  ...
  const body = readFileSync28(sourcePath, "utf8");
...
import { existsSync as existsSync27, statSync as statSync4 } from "node:fs";
...
async function currentGitBranch(root) {
  if (!existsSync27(join30(root, ".git"))) {
    return null;
  }
...
function recordAdapterEvent(root, kind, attrs) {
  try {
    appendRecord(join31(projectStateDir(root), "obs.jsonl"), {
      schema: "harness.observability.v1",
      provider: "unknown",
      kind,
      level: "signal",
...
var sessionStartHandler = async (event, ctx) => {
  ...
  const handoff = handoffSeal.ok ? coreFacade.handoff.readHandoff(root, event.provider) : {};
  const foreign = handoffSeal.ok ? coreFacade.handoff.readForeignSlices(root, event.provider) : [];
  const lines = [
    ...[handoffSeal.note, lessonSeal.note].filter((note) => note !== null),
