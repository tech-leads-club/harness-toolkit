...
// bin/tlc-exec.mjs
import { existsSync as existsSync26, mkdirSync as mkdirSync17, readFileSync as readFileSync28, realpathSync as realpathSync2, writeFileSync as writeFileSync16 } from "node:fs";
import { delimiter as delimiter2, dirname as dirname7, join as join27 } from "node:path";
...
function runtimeCachePath(harnessHome) {
  return join27(harnessHome, "state", "runtime-cache.json");
}
function writeRuntimeCache(harnessHome, bunPath) {
  ...
  try {
    ...
    mkdirSync17(dirname7(cachePath), { recursive: true });
    writeFileSync16(cachePath, `${JSON.stringify(record)}
`);
  } catch {}
  return record;
}
...
import { existsSync as existsSync28, mkdirSync as mkdirSync19, readFileSync as readFileSync30, writeFileSync as writeFileSync18 } from "node:fs";

// src/providers/claude/claude.wiring.ts
import { existsSync as existsSync27, mkdirSync as mkdirSync18, readFileSync as readFileSync29, realpathSync as realpathSync3, writeFileSync as writeFileSync17 } from "node:fs";
...
function claudeSettingsPath() {
  return join28(claudeConfigDir(), "settings.json");
}
...
function cursorWiring(runtime) {
  ...
  return {
    target: join29(cursorConfigDir(), "hooks.json"),
    strategy: "replace",
    entries
  };
}
...
function isCursorWired(targetPath) {
  return existsSync28(targetPath) && readFileSync30(targetPath, "utf8").includes(CURSOR_MARKER);
}
if (false) {}
