...
import { existsSync as existsSync26, mkdirSync as mkdirSync16, renameSync as renameSync2, rmSync as rmSync4, writeFileSync as writeFileSync15 } from "node:fs";
...
// tools/refresh-model-prices.ts
var HARNESS_HOME = runtimeHome();
var PROVIDER_PLANE = "cursor";
var PROVIDER_DOCS_URL = "https://cursor.com/docs/models-and-pricing.md";
...
function adoptLegacyOverrides(quiet) {
  ...
  if (existsSync26(overridesPath())) {
    console.error(`refresh: ${basename5(path)} holds ${keys.length} legacy entries and ${basename5(overridesPath())} already exists — merge them by hand`);
    return;
  }
  renameSync2(path, overridesPath());
  if (!quiet) {
    console.log(`refresh: moved ${keys.length} local overrides → ${overridesPath()}`);
  }
}
function retireSupersededFiles(quiet) {
  for (const name of SUPERSEDED) {
    const path = join27(HARNESS_HOME, name);
    if (!existsSync26(path)) {
      continue;
    }
...
function planeCount(catalogue, plane) {
  return Object.keys(catalogue.planes?.[plane] ?? {}).length;
}
function applyPlanes(existing, updates, now) {
  const planes = { ...existing.planes ?? {} };
...
async function main() {
  ...
  const path = cataloguePath();
  function wanted(plane, label) {
    if (!ifStale) {
      return true;
    }
    const meta = existing._meta?.planes?.[plane];
    const state = coreFacade.pricing.freshness(existing.planes?.[plane] === undefined ? null : meta ?? {}, new Date);
  ...
  mkdirSync16(HARNESS_HOME, { recursive: true });
  ...
  if ((mode === "all" || mode === FALLBACK_PLANE) && wanted(FALLBACK_PLANE, `${FALLBACK_PLANE} prices`)) {
    ...
    updates.push({
      ...
      source: LITELLM_URL,
      table: parseLiteLlm(await res.json())
    });
  }
  if (updates.length === 0) {
    return;
  ...
  if (outcomes.some((outcome) => outcome.accepted)) {
    writeFileSync15(path, `${JSON.stringify(catalogue)}
`);
