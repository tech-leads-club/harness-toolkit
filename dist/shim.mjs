...
var handler = requireHandler();
function coveringSettings() {
  let seen = null;
...
var decision = coreFacade.shim.decideShim(coveringSettings(), handler);
if (!decision.run) {
...
function run(command, args) {
  const child = spawn2(command, args, { stdio: ["pipe", "pipe", "inherit"], env: process.env });
