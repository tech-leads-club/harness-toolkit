...
| **Comment gate (agent-added comments)**<br>`comments.enabled` · off | Checks the comment lines this turn added against the commit the turn started from, by declared reason or none at all. | `stop` | `block-stop` | tlc harness obs report — the comments gate appears among the gate outcomes |
