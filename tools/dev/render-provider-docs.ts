...
//
// why the two opencode rows: one host, one document, two ports — the plugin API generations differ in what the
// harness can do, so they are separate registry entries ([/decisions/ad-146.md](/decisions/ad-146.md)) documented
// side by side.
const DOC_FILE_OVERRIDES: Record<string, string> = {
  claude: "claude-code.md",
  "opencode-legacy": "opencode.md",
  "opencode-namespaced": "opencode.md",
};

export function docFileFor(name: string): string {
  return join("docs", "providers", DOC_FILE_OVERRIDES[name] ?? `${name}.md`);
}

// invariant: a document that serves one provider keeps the bare marker every existing doc already carries. Only a
// shared document qualifies it, because two tables cannot both live under `generated:capabilities`.
export function capabilityRegionFor(name: string): string {
  const file = docFileFor(name);
  const sharing = providers.filter((provider) => docFileFor(provider.name) === file);
  return sharing.length > 1 ? `capabilities:${name}` : "capabilities";
}
...
export async function renderAll(root = repoRoot): Promise<{ file: string; current: string; next: string }[]> {
  ...
  // hazard: two providers can share one document. Rendering each against the file on disk would have the second
  // result silently revert the first one's region when both are written — the defect `render-capabilities.ts`
  // records for the README. Each provider renders on top of what the previous one left for that file.
  const renderedByFile = new Map<string, string>();
  for (const provider of providers) {
    ...
    let next = renderedByFile.get(file) ?? current;
    try {
      next = replaceRegion(
        next,
        capabilityRegionFor(provider.name),
        renderCapabilityTable(provider.capabilities()),
      );
    } catch (error) {
