...
describe("appendProviderToRegistry", () => {
  // why entries and not a literal: the registry grows, and the formatter wraps the array once it stops fitting
  // on a line. What this pins is the order, which is detection order, not how many hosts there happen to be.
  function registryEntries(text: string): string[] {
    const body = /export const providers: ProviderPort\[\] = \[([^\]]*)\];/.exec(text)?.[1] ?? "";
    return body
      .split(",")
  ...
  test("appends the new provider after the existing ones, unchanged in order", () => {
    const current = readFileSync(join(repoRoot, "src", "providers", "provider.registry.ts"), "utf8");
    const before = registryEntries(current);
    assert.ok(before.length >= 2, "the real registry lists its providers");

    const next = appendProviderToRegistry(current, "acme");

    assert.match(next, /import \{ acmeProvider \} from "\.\/acme\/index\.ts";/);
    assert.deepEqual(registryEntries(next), [...before, "acmeProvider"]);

    const oneLine = "export const providers: ProviderPort[] = [cursorProvider, claudeProvider];";
    assert.match(
      appendProviderToRegistry(`import type { ProviderPort } from "./provider.port.ts";\n${oneLine}`, "acme"),
      /= \[cursorProvider, claudeProvider, acmeProvider\];/,
    );
